...
\begin{verbatim}
    ...
    sub x3, x1, x2
\end{verbatim}
Figure \ref{fig:FWD_alu_alu} shows the result of the simulation. Here, the first operand of the \texttt{sub} instruction, \texttt{x1}, is taken from the MEM/WB pipe register, since it was set two instruction earlier. Therefore, in the clock cycle indicated by the gold cursor, \texttt{FWU\_fwdA = 1}. The second operand, \texttt{x2}, is forwarded from the previous ALU result in the EX/MEM pipe register, so \texttt{FWU\_fwdB = 2}. Both the sel signals are coloured in pale blue, while the outputs of these multiplexers are in purple. They are set correctly even if the corresponding register has not been written yet. As a result, the alu output of the third instruction in the EX stage (\texttt{ALU\_out}) is correct: $-44 - 56 = -100$.

\begin{figure}[hbtp]
    \centering
    \includegraphics[width=\textwidth]{../fwu/ref/simResult/FWD_alu_alu.pdf}
    \caption{Forwarding: ALU instruction after ALU instruction}
    \label{fig:FWD_alu_alu}
\end{figure}
...
\begin{verbatim}
    ...
    sw x2, 32(x1)
\end{verbatim}
Again, there's no need to stall the pipeline since the value loaded from the data memory is forwarded to its data input before it is actually written to register \texttt{x2}. This kind of forwarding is particularly useful when a data in a memory location has to be moved to another location. This operation can be done in just two clock cycle. The wave postscript of this execution is reported in figure \ref{fig:FWD_load_store}.

\begin{figure}[hbtp]
    \centering
    \includegraphics[width=\textwidth]{../fwu/ref/simResult/FWD_load_store.pdf}
    \caption{Forwarding: Store instruction after load instruction}
    \label{fig:FWD_load_store}
\end{figure}
...
\begin{verbatim}
    ...
    addi x1, t0, 1 
\end{verbatim}
This is the only case a data hazard cannot be resolved by forwarding, as mentioned in section \ref{sec:load_store_fwd}. The pipeline must be stalled. This is shown in figure \ref{fig:FWD_load_alu}: the \texttt{stall\_n} signal is asserted and the program counter (\texttt{I\_MEM\_addr}) is prevented from incrementing to the next instruction, so that the second one is fetched again in the next cycle, when the value loaded from the memory is available, even before it is actually written to register \texttt{t0} (\texttt{x5}, at the bottom of the wave figure), thanks to the forwarding logic instantiated into the register file. This example shows the perfect collaboration between the FWU and the HDU, and how they manage to speed up the processor together. Good guys they are. 

\begin{figure}[hbtp]
    \centering
    \includegraphics[width=\textwidth]{../fwu/ref/simResult/FWD_load_alu.pdf}
    \caption{Forwarding: ALU instruction after load. The pipeline is stalled}
    \label{fig:FWD_load_alu}
\end{figure}
...
\includepdf[pages={1-2}]{img/min_calc.pdf}

% ASSEMBLER section 
% \label{sec:assembler}
\input{sec/assembler.tex}
